...
const Interns = () => {
  ...
  const [activeTab, setActiveTab] = useState<'manager' | 'team-member' | 'intern' | null>(null);
  ...
  const handleTabClick = (tab: 'manager' | 'team-member' | 'intern') => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent, tab: 'manager' | 'team-member' | 'intern') => {
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-slate-300">
              Help us improve by completing the appropriate survey
            </p>
          </div>
          ...
          <div
            role="tablist"
            aria-label="Survey Feedback Tabs"
          ...
          >
            <button
              ...
              className={`px-6 py-3 rounded-lg font-semibold text-base md:text-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-950 ${
                activeTab === 'manager'
                  ? 'bg-gradient-to-r from-white to-blue-900 text-slate-900 shadow-lg shadow-blue-500/50 scale-105'
            ...
            <button
              role="tab"
              aria-selected={activeTab === 'team-member'}
              aria-controls="team-member-panel"
              id="team-member-tab"
              tabIndex={activeTab === 'team-member' ? 0 : -1}
              onClick={() => handleTabClick('team-member')}
              onKeyDown={(e) => handleKeyDown(e, 'team-member')}
              className={`px-6 py-3 rounded-lg font-semibold text-base md:text-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-950 ${
            ...
            <button
              ...
              aria-controls="intern-panel"
              id="intern-tab"
              tabIndex={activeTab === 'intern' ? 0 : -1}
              onClick={() => handleTabClick('intern')}
              onKeyDown={(e) => handleKeyDown(e, 'intern')}
              className={`px-6 py-3 rounded-lg font-semibold text-base md:text-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-950 ${
                activeTab === 'intern'
                  ? 'bg-gradient-to-r from-white to-blue-900 text-slate-900 shadow-lg shadow-blue-500/50 scale-105'
                  : 'bg-slate-800/50 text-slate-300 hover:bg-slate-700/70 hover:text-white hover:scale-102'
          ...
          <div className="relative min-h-[600px]">
            {/* Manager Survey Panel */}
            <div
              role="tabpanel"
              id="manager-panel"
              aria-labelledby="manager-tab"
              hidden={activeTab !== 'manager'}
              className={`bg-slate-900/50 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden border border-slate-700/50 transition-all duration-500 ${
                activeTab === 'manager' ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
              }`}
            >
            ...
            {/* Team Member Survey Panel */}
            <div
              role="tabpanel"
              id="team-member-panel"
              aria-labelledby="team-member-tab"
              hidden={activeTab !== 'team-member'}
              className={`bg-slate-900/50 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden border border-slate-700/50 transition-all duration-500 ${
                activeTab === 'team-member' ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
              }`}
            >
              <div className="relative w-full" style={{ paddingBottom: '80%' }}>
                <iframe
            ...
            <div
              ...
              hidden={activeTab !== 'intern'}
              className={`bg-slate-900/50 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden border border-slate-700/50 transition-all duration-500 ${
                activeTab === 'intern' ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
              }`}
            >
              <div className="relative w-full" style={{ paddingBottom: '80%' }}>
                <iframe
